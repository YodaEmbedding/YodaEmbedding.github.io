...
Thus, if \\( X\_1, X\_2, \ldots, X\_n \\) are truly independently and identically distributed (i.i.d.), then this codebook is the optimal codebook.
...
In data compression, this process of determining the encoding distribution on-the-fly based on previous information is known as *context modeling*; and more generally, for any way of determining the encoding distribution, as *entropy modeling*.
...
The rate cost for encoding \\( \hat{y}\_i \\) is the negative log-likelihood, \\( R\_{{\hat{y}}\_i} = -\log p\_{{\hat{y}}\_i}({\hat{y}}\_i) \\), measured in bits.
...
The exact total rate cost for encoding \\( \boldsymbol{\hat{y}} \\) using \\( p\_{\boldsymbol{\hat{y}}} \\) is simply the sum of the negative log-likelihoods of each element, \\( R\_{\boldsymbol{\hat{y}}} = \sum\_i -\log p\_{{\hat{y}}\_i}({\hat{y}}\_i) \\).
...
Furthermore, in the case of the Gaussian conditional hyperprior used by Ballé *et al.*, the encoding distributions are Gaussian distributions parameterized by a mean and variance.
`
>}}
...
-   A *Gaussian conditional*, as introduced by Ballé *et al.* [^ref-balle2018variational].
    ...
    Then, like in (1),
    <!-- {{< cref "eqn:p_y_c_integral" >}}, -->
    the encoding distribution \\( p\_{\boldsymbol{\hat{y}}\_i} \\) is the binned area under \\( f\_i \\):
    \\[
    \begin{equation}
